...
std::map<std::string, std::shared_ptr<Mod>> loadDllFiles() {
  ...
  for (const auto &dirEntry :
    ...
    if (!temp) {
      Traveller::log("Couldn't load library " + path);
      continue;
    }

    Traveller::log("Loading library " + path);
    ...
    if (address != nullptr) {
      ...
      Traveller::log("Loaded mod " + mod->getName());

      mods[mod->getName()] = std::shared_ptr<Mod>(mod);
    } else {
      Traveller::log("No mod found in " + path);
    }
  }
  Traveller::log("Loaded " + std::to_string(mods.size()) + " mods.");
...
void lateRender() {
  ...
  Traveller::currentModule = "GUIManager";
...
std::map<std::string, std::shared_ptr<Mod>> getMods() {
   return loadedMods;
}
} // namespace ScriptingLibrary

TRAVELLER_API_REGISTRY() { lua.set_function("log", Traveller::log); }

...
template <void (*F)(), uintptr_t TargetFunc> void callTemplate() {
  typedef void (*fptr)();
...
TTSLLib void injectFunction() {
  ...
  auto last =
      MemWriteUtils::setMemoryPerms(CallInstr, 8, PAGE_EXECUTE_READWRITE);
  *reinterpret_cast<int *>(replacementOffset) = reljmp;
  MemWriteUtils::setMemoryPerms(CallInstr, 8, last);
}

TTSLLib inline void replaceDllFunction(const std::string &dllName,
                                       const std::string &functionName,
                                       void *newFunction) {
  std::wstring dllwName = std::wstring(dllName.begin(), dllName.end());
  auto result = MH_CreateHookApi(dllwName.c_str(), functionName.c_str(),
                                 newFunction, nullptr);

  if (result != MH_OK) {
    std::cout << "Failed to disable " << dllName << "/" << functionName << ": "
              << MH_StatusToString(result) << std::endl;
  }
